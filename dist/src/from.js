...
const { filterPseudoHeaders, copyHeaders, stripHttp1ConnectionHeaders, buildURL, } = require("./utils");
const { TimeoutError } = buildRequest;
exports.default = fp(function from(fastify, opts, next) {
    const contentTypesToEncode = new Set([
        "application/json",
        ...(opts.contentTypesToEncode || []),
    ]);
    const retryMethods = new Set(opts.retryMethods || ["GET", "HEAD", "OPTIONS", "TRACE"]);
    ...
    fastify.decorateReply("from", function (source, opts) {
        ...
        const _a = this.request.body || {}, { proxy_key, proxy_cert, rewrite_method } = _a, restBody = __rest(_a, ["proxy_key", "proxy_cert", "rewrite_method"]);
        ...
        if (proxy_cert && proxy_key) {
            opts.body = req.method !== 'GET' ? Object.assign({}, restBody) : undefined;
            opts.http = Object.assign(Object.assign({}, opts.http), { agents: {
        ...
        this.request.log.info({ source, method: req.method, headers }, "fetching from remote server");
        ...
        requestImpl({ method: req.method, url, qs, headers: requestHeaders, body }, (err, res) => {
            ...
            if (sourceHttp2) {
                copyHeaders(rewriteHeaders(stripHttp1ConnectionHeaders(res.headers), req), this);
            }
            else {
    ...
    fastify.addHook("onReady", (done) => {
        if (isFastifyMultipartRegistered(fastify)) {
            fastify.log.warn("@dmikey/fasity-mtls-proxy/reply-from might not behave as expected when used with @fastify/multipart");
        }
        done();
    });
...
function isFastifyMultipartRegistered(fastify) {
    return (fastify.hasContentTypeParser("multipart") &&
